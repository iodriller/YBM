...
no visibility short of opening it in a SQLite browser (see docs/ROADMAP.md P6).
"""
...
# task_signals/subtasks/tool_invocations/plans/artifacts/approvals reference
# tasks; tasks/messages/conversation_memory reference conversations. audit_events
# and schedules carry no FK. See backend/src/agent_control/storage/migrations.py.
TABLES_CHILD_FIRST = (
        ...
        if "schedules" in table_names:
            ...
            if schedule_count:
                ...
                print(f"schedules: {schedule_count} active (run `ybm db inspect` again after "
                      f"changes; stale schedules keep spawning tasks - see docs/ROADMAP.md P6)")
    return 0
...
def db_clean(days: int) -> int:
    ...
    with database.connect() as connection:
        ...
        # Only tasks are used as the retention anchor: delete tasks older than
        # the cutoff, then cascade-delete their children by task_id so we
        # never orphan audit/tool-call history for a task that's kept.
        if "tasks" not in tables:
            print("no tasks table found - nothing to clean")
            return 0
        old_task_ids = [
            row["id"]
        ...
        print(f"deleted {len(old_task_ids)} task(s) and their child records")
    return 0
