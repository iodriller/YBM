  ...
  return (
    <AdvancedModeContext.Provider value={{ advanced, setAdvanced }}>
      <div className="flex h-svh w-full flex-col overflow-hidden bg-background text-foreground">
        ...
        <nav className="fixed inset-x-0 bottom-0 z-40 grid h-17 grid-cols-4 border-t border-sidebar-border bg-sidebar/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden">
          {NAV_ITEMS.map(({ to, label, icon: Icon, end }) => (
            <NavLink
              key={to}
              to={to}
