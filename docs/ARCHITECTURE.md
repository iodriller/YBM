...
| LLM provider catalog | `llm/catalog.py` | The 13 providers the console offers, as data: base URL, key env var, default model. Adding a provider is a row here, not a code path. `llm/providers.py::build_provider_for_profile` is the one place a profile's `provider` field maps to an implementation |
| Anthropic provider | `llm/anthropic_provider.py` | Native, because Anthropic's API is not OpenAI-compatible. Omits `temperature` on models that reject it with a 400 (Opus 5, Sonnet 5, Opus 4.7/4.8, Fable 5), puts the system prompt top-level, forces a tool call for structured output, and checks `stop_reason` before reading content. Its 5xx wording deliberately matches what `FailoverLLMProvider` greps for |
| Hardware probe | `llm/hardware.py` | Asks LocalDeploy's `GET /system/hardware` first (it reports *free* VRAM and multiple GPUs), falling back to nvidia-smi. Drives the fit verdict on each local preset - fits / too_big / unknown - so "runs on this machine" is earned rather than asserted. "Unknown" is a first-class answer |
| Channel catalog | `channels/catalog.py` | Every way to reach YBM with an honest status. `connected` is resolved per request from real config, so a Telegram token with an empty allowlist is not reported as connected - that combination produces a bot that ignores every message |
| User-facing errors | `error_text.py` | Two audiences, deliberately separated: `describe_exception()` produces `TypeName: message` for logs and the task trace, `explain_for_user()` maps known failure shapes to a sentence on what happened and a sentence on what to do. Accepts a stored string as well as a live exception, because `last_worker_error` is a string by the time anyone reads it |
